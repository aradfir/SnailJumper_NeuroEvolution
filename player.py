...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [6,10, 2]  # TODO (Design your architecture here by changing the values)
            self.nn = NeuralNetwork(layer_sizes)

    square_size = (431 - 176) / 5
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        # inputs = np.zeros((15, 5))
        # for i in range(len(obstacles)):
        #     y_grid =  (obstacles[i]['y'] + 100) // self.square_size
        #     # if y_grid >= 10:
        #     #     continue
        #     x_grid = (obstacles[i]['x'] - 176) // self.square_size
        #     inputs[int(y_grid), int(x_grid)] = -2
        # x_grid = (player_x - 176) // self.square_size
        # y_grid =  (player_y + 100) // self.square_size
        # inputs[int(y_grid), int(x_grid)] += 1
        # inputs = inputs.reshape(75)

        # input[0] snail on the side of player
        # input[1] snail on the opposite side of player
        # input [2,3] is fly




        nn_inputs = np.zeros(6)
        flags = [False, False, False]
        for i in range(len(obstacles)):
            if obstacles[i]['x'] < 180:
                if not flags[0]:
                    flags[0] = True
                    nn_inputs[0] = np.abs(player_x - obstacles[i]['x'])/screen_width
                    nn_inputs[1]= (player_y - obstacles[i]['y'])/screen_height

            elif obstacles[i]['x'] > 420:
                if not flags[1]:
                    flags[1] = True
                    nn_inputs[2] = np.abs(player_x - obstacles[i]['x'])/screen_width
                    nn_inputs[3] = (player_y - obstacles[i]['y']) / screen_height
            else:
                if not flags[2]:
                    flags[2] = True
                    nn_inputs[4] = np.abs(player_x - obstacles[i]['x']) / screen_width
                    nn_inputs[5] = (player_y - obstacles[i]['y']) / screen_height
